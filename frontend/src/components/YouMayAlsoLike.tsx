...
const YouMayAlsoLike = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await apiRequest(`${API_ENDPOINTS.PRODUCTS.LIST}?limit=6`);
    ...
    <section className="max-w-7xl mx-auto px-6">
      ...
      <div className="flex flex-col gap-1 sm:gap-2 mb-6 sm:mb-8">
        <div className="flex items-center justify-between">
          <h2 className="text-lg sm:text-2xl font-bold text-gray-900 leading-tight tracking-tight">
            You May Also Like
          </h2>
          <button 
        ...
        <span className="text-xs sm:text-sm text-gray-500">
          Some of the recommended products for you
        </span>
      </div>
