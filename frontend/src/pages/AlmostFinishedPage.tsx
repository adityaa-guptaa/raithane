...
const AlmostFinishedPage = () => {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      // Fetch more products initially to increase improved client-side filtering yield
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/products?limit=50`
      );
      const data = await response.json();
      
      // Filter for products with less than 100 items in stock
      // and ensure stock is greater than 0 (so we don't just show out of stock items, unless desired)
      // Assuming 'almost finished' implies low stock but available.
      // If user wants filtered by < 100 regardless of 0, we can adjust.
      // Let's assume < 100 and > 0 for now to be useful.
      // If the backend supported filtering by stock, that would be better.
      const lowStockProducts = (data.products || []).filter((p: any) => p.stock < 100 && p.stock > 0);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="bg-gradient-to-r from-[#992A16] to-[#be3c25] text-white py-12 lg:py-20 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-lg lg:text-2xl mb-6 opacity-90">
            Hurry up! These items are running out fast.
          </p>
          <p className="text-base lg:text-lg mb-8 opacity-80 max-w-2xl mx-auto">
            Grab your favorites before they are gone forever. Limited stock available!
          </p>
          <button
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 lg:py-20">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Items with less than 100 units left in stock.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">No low stock products found at the moment.</p>
          </div>
        ) : (
            ...
            {products.map((product) => (
              ...
              >
                ...
                >
                  ...
                  />
                  {/* Low Stock Badge */}
                  <div className="absolute top-2 left-2 bg-orange-500 text-white px-2 py-0.5 rounded-full text-xs font-semibold animate-pulse">
                    Low Stock
                  </div>
      ...
      <section className="bg-[#f8f6f2] py-12 lg:py-16 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-gray-600 text-lg mb-8 max-w-2xl mx-auto">
            These essential items are flying off the shelves. Secure yours now before they go out of stock!
          </p>
          <button
...
export default AlmostFinishedPage;
